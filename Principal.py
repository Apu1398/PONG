...
ball=Rect(100,100, 30,30 )
dx=2
dy=2
#bola=Bola(rect, LARGO_VENTANA, ANCHO_VENNTANA)
...
#pygame.key.set_repeat(2, 100) #Instruccion que arregla la fluidez de la teclas
while True:                                                    #Bucle principal
    ...
    ball.move_ip(dx, dy)
    if ball.y < 0 or ball.y > LARGO_VENTANA-40:
        dy *= -1 #se multiplica por -1 porque cualquier numero multiplicado por -1 da el negativo

    if ball.x >ANCHO_VENNTANA-40:
        dx*=-1
    if ball.x<0:
        dx*=-1


    pygame.draw.ellipse(ventana, (222,22,34), ball)
